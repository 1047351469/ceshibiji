...
```
	...
	参数：
		...
		3.end_x：  终点X轴坐标
		4.end_y,： 终点Y轴坐标
		5.duration： 滑动这个操作一共持续的时间长度，单位：ms（时间越长越精准）
```
```
	业务场景：
		1.进入设置
...
```
	代码实现：
	    ...
	    el2 = driver.find_element_by_xpath("//*[contains(@text,'WLAN')]")
	    # 执行滑动操作
	    driver.drag_and_drop(el1,el2)（没有惯性，更精准，漏出一点点，也能找到元素）
```
...
```
```
	方法：get_window_size() width 1/2 height 1/4
```
```
	代码实现：
